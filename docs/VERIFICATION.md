...
| CAC-12 | Claim-safe native-window doctor/status, exit codes, schemas, and eight golden scenarios | Local gate passed; commit and remote CI pending | `src/doctor.rs`, doctor/status schemas, golden hashes, 8 doctor tests, and live read-only baseline proof |
